...
BrowserWorld::State::UpdateControllers(bool& aRelayoutWidgets) {
  ...
  for (Controller& controller: controllers->GetControllers()) {
    ...
    if (controller.pointer) {
      bool isGaze = device->IsInGazeMode() && (controller.index == device->GazeModeIndex());
      controller.pointer->SetVisible((hitWidget.get() != nullptr || isGaze) && controller.hasAim);
      ...
      if (controller.hasAim) {
        if (hitWidget) {
          ...
          // Project pointer at a fixed distance (e.g. 2.0 meters) if not hitting any widget in gaze mode
          vrb::Vector fixedHitPoint = start + direction * 2.0f;
          vrb::Matrix translation = vrb::Matrix::Translation(fixedHitPoint);
          // For gaze without a normal, point the cursor back at the user (Billboard)
          vrb::Matrix localRotation = vrb::Matrix::Rotation(-direction);
          vrb::Matrix reorient = rootTransparent->GetTransform();
          controller.pointer->SetTransform(reorient.AfineInverse().PostMultiply(translation).PostMultiply(localRotation));
          const float scale = (fixedHitPoint - device->GetHeadTransform().MultiplyPosition(vrb::Vector(0.0f, 0.0f, 0.0f))).Magnitude();
          controller.pointer->SetScale(scale + kPointerSize);
          controller.pointer->SetPointerColor(VRBrowser::GetPointerColor());
        }
      }
    }
