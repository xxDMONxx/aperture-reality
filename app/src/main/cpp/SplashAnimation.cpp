...
SplashAnimation::Update(const vrb::Matrix& aHeadTransform) {
  ...
  if (m.time < 0.4f) {
    alpha = m.time / 0.4f;
  } else if (m.time >= SPLASH_SECONDS && m.time <= (SPLASH_SECONDS + FADE_OUT_TIME)) {
    alpha = 1.0f - (m.time - SPLASH_SECONDS) / FADE_OUT_TIME;
  }
  m.logo->SetTintColor(vrb::Color(alpha, alpha, alpha, alpha));
